...
export default function ReservationsModal({ onClose, reservations = [] }) {
    ...
    const getDisplayTitle = () => {
        if (selectedDate !== null) {
            const dayName = new Date(currentDate.getFullYear(), currentDate.getMonth(), selectedDate).toLocaleString("default", { weekday: "long" })
            return `Reservations for ${dayName} ${selectedDate} ${monthName.split(" ")[0]}`
        }
                                ...
                                {weekDates.map((date, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`week-day-card ${isSelected ? "selected" : ""} ${hasReservations ? "has-reservations" : ""}`}
                                        >
                                            <div className="day-name">{getDayName(date)}</div>
                                            <div className="day-number">{dayNum}</div>
                                            {hasReservations && (
                                                <div className={`booking-count ${isSelected ? "selected-text" : ""}`}>
                                                    {reservationsMap[dayNum].length} booking{reservationsMap[dayNum].length > 1 ? "s" : ""}
                                                </div>
                                            )}
                                        </button>
                                    )
                        ...
                        {showCalendar && (
                            <div className="calendar-container">
                                ...
                                <div className="calendar-days">
                                    {emptyDays.map((_, idx) => <div key={`empty-${idx}`} className="empty-day"></div>)}
                                    {days.map((day) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`calendar-day ${selectedDate === day ? "selected" : ""} ${hasReservations ? "has-reservations" : ""}`}
                                            >
                                                {day}
                                            </button>
                                        )
                        ...
                        <div>
                            ...
                            <div className="reservations-list">
                                {displayReservations.length > 0 ? (
                                        ...
                                        <div key={res.id} className="reservation-card">
                                            ...
                                            <div className="res-footer">
                                                <div className="res-contact">
                                                    <span>📞</span>
                                                    <span className="res-contact-text">{res.contact_info}</span>
                                                </div>
                                    ...
                                    <div className="no-results"><p>No reservations for this date</p></div>
                                )}
                            </div>
                        </div>
                    </div>
